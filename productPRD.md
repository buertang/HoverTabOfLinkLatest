...
- **隐私保护**: 严格遵循零数据收集原则，所有设置仅使用浏览器提供的存储（包含同步存储）保存，不会上传到任何服务器。
...
- **设置存储**: 使用 chrome.storage.sync 跨设备同步用户配置；若网络或配额异常将自动回退至 chrome.storage.local，保证可用性
- **状态管理**: React状态管理 + 自定义Hooks
- **数据同步**: 依托浏览器账号进行自动同步，无需额外账号
- **数据验证**: 完整的数据格式验证和错误恢复
